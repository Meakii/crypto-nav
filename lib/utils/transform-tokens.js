...
export const kebabCase = (str) =>  str && str
...
// Add new transform for PX suffix
StyleDictionaryPackage.registerTransform({
  name: "value/pxSuffix",
  type: "value",
  matcher: function (token) {
    // Match tokens that start with these prefixes
    return (
  ...
  transformer: function (token) {
    // If the value already ends with px, return as is
    if (token.value.toString().endsWith("px")) {
      return token.value;
    }
    // If the value is 0, return just 0 without px
...
//       return `${type === "innerShadow" ? "inset " : ""}${x / BASE_REM}rem ${
//         y / BASE_REM
//       }rem ${blur / BASE_REM}rem ${color} `;
//     });
//     return value.join(", ");
//   },
// });
